...
const VoiceAssignmentStudent = () => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      const res = await axios.get('http://localhost:5001/api/student/assignments');
      if (Array.isArray(res.data)) apiItems.push(...res.data);
    } catch (e) {}
  ...
  const processVoiceCommand = (spokenText) => {
    ...
    // Command when not in active test mode
    if (!active) {
      if (lower.includes('start exam') || lower.includes('start test') || lower.includes('start assignment') || lower.includes('start voice test')) {
        const available = assignmentsRef.current;
        if (available && available.length > 0) {
          handleStartAssignment(available[0]);
          setTranscript('');
        }
      }
      return;
    ...
    if (!active.questions[cIdx]) return;
    ...
    if (lower.includes('skip') || lower.includes('skip question')) {
      handleNextQuestion();
      setTranscript('');
      return;
    }
    if (lower.includes('submit') || lower.includes('finish') || lower.includes('done') || lower.includes('submit assignment') || lower.includes('submit exam') || lower.includes('finish exam')) {
      handleSubmitAssignment();
      setTranscript('');
      return;
    }
    if (lower.includes('exit') || lower.includes('exit exam') || lower.includes('exit test') || lower.includes('exit assignment')) {
      handleExitAssignment();
      setTranscript('');
      return;
    }
