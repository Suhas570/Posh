...
export const HRResults = ({ onViewDetail }) => {
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm text-left">
      ...
      {results.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 p-12 text-center text-slate-400">
          ...
          <p className="text-slate-400 text-xs mt-1">Results will appear here automatically when candidates submit their exams.</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50/50">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
              {results.map((row) => (
                <tr key={row.id} className="hover:bg-slate-100/40 transition-colors duration-150">
                  <td className="py-4 px-6">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-800">{row.candidateName}</span>
                      <span className="text-xs text-slate-500 mt-1 font-semibold">{row.candidateEmail}</span>
                    </div>
                  </td>
                      ...
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold border w-fit ${
                          row.passFail === 'PASS'
                            ...
                            : 'bg-rose-55/60 bg-rose-50 border-rose-200 text-rose-700'
                        }`}
                      >
                        {row.passFail === 'PASS' ? (
                          <>
                            <Award className="h-3.5 w-3.5 text-emerald-500" /> Pass
                          </>
                        ) : (
                          <>
                            <Frown className="h-3.5 w-3.5 text-rose-500" /> Fail
                          </>
                        )}
                      </span>
                      {row.autoSubmitted && (
                  ...
                  <td className="py-4 px-6 text-right">
                    <button
                      onClick={() => onViewDetail(row.attemptId)}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 px-3 py-2 text-xs font-bold text-slate-700 transition-all cursor-pointer"
                    >
