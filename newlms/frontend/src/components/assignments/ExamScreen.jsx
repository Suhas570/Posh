...
export const ExamScreen = ({ attemptId, onFinished }) => {
  ...
  if (error && !questions.length) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-955/20 bg-slate-950 p-6 text-left">
        <div className="rounded-xl border border-rose-900 bg-rose-950/20 p-8 text-center max-w-md">
  ...
  return (
    <div className="flex flex-col h-[85vh] bg-slate-950 text-slate-350 rounded-3xl overflow-hidden relative border border-slate-800 text-left">
      {/* Header */}
      <header className="flex items-center justify-between border-b border-slate-800 bg-slate-900/50 px-6 py-4 shrink-0">
        <div className="flex items-center gap-4">
          <div className="font-bold text-white text-md tracking-tight">Technical Assessment</div>
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-rose-950/30 border border-rose-900/50 rounded-full text-[10px] font-bold text-rose-400">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {[0, 1].map(i => (
                <div key={i} className={`h-2 w-5 rounded-full ${i < warnings.length ? 'bg-rose-500' : 'bg-slate-800'}`} />
              ))}
            </div>
          </div>
          <button
            ...
            disabled={submitting}
            className="rounded-xl bg-indigo-600 hover:bg-indigo-700 px-6 py-2 text-xs font-bold text-white shadow transition-all disabled:opacity-50 cursor-pointer"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-16 sm:w-56 border-r border-slate-800 bg-slate-950/20 flex flex-col">
          {/* Camera Feed */}
          <div className="p-3 border-b border-slate-800 bg-black aspect-video relative shrink-0 flex items-center justify-center">
            ...
            {!modelsLoaded && <div className="z-10 text-[9px] text-indigo-400 font-bold bg-black/60 px-2 py-1 rounded">Loading Proctoring...</div>}
          ...
          <div className="flex-1 overflow-y-auto p-4 hidden sm:block">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-4">Questions Map</h3>
            <div className="grid grid-cols-4 gap-2">
              {questions.map((q, idx) => {
                ...
                let btnClass = 'h-8 w-full rounded-lg border text-xs font-bold transition-all flex items-center justify-center cursor-pointer ';
                if (isCurrent) btnClass += 'bg-indigo-600 border-indigo-500 text-white shadow-sm';
                else if (isAnswered) btnClass += 'bg-emerald-950/30 border-emerald-900/50 text-emerald-400';
                else btnClass += 'bg-slate-900 border-slate-800 text-slate-400 hover:bg-slate-850';
        ...
        {/* Content Area */}
        <div className="flex-1 flex flex-col bg-slate-900/30 overflow-hidden relative">
          {currentQ && (
            <div className="flex-1 flex flex-col p-6 overflow-y-auto">
              <div className="flex items-center gap-3 mb-6">
                <span className="flex h-7 items-center justify-center rounded-lg bg-indigo-950/60 border border-indigo-900/50 px-2.5 text-xs font-black text-indigo-400">
                  Q{currentIdx + 1}
                </span>
                <span className="text-[9px] font-black uppercase bg-slate-800 text-slate-400 px-2 py-0.5 rounded">
                  {currentQ.type}
                </span>
                <span className="text-[9px] font-black uppercase bg-indigo-950/40 border border-indigo-900/50 text-indigo-400 px-2 py-0.5 rounded">
                  {currentQ.marks} Marks
                </span>
              </div>

              <div className="text-md font-semibold text-white mb-6 whitespace-pre-wrap leading-relaxed">
                {currentQ.text}
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left flex items-center gap-4 rounded-xl border p-4 transition-all duration-200 cursor-pointer ${
                          isSelected
                            ? 'bg-indigo-955/20 border-indigo-500 shadow-sm'
                            : 'bg-slate-950 border-slate-800 hover:border-slate-700 hover:bg-slate-900'
                        }`}
                      >
                        <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold ${
                          isSelected ? 'border-indigo-500 bg-indigo-500 text-white' : 'border-slate-700 bg-slate-900 text-slate-400'
                        }`}>
                          {String.fromCharCode(65 + oIdx)}
                        </div>
                        <span className={`text-xs ${isSelected ? 'text-white font-semibold' : 'text-slate-350'}`}>
                          {opt}
                        </span>
                      </button>
                    );
                ...
                <div className="flex-1 flex flex-col min-h-[300px] gap-4">
                  <div className="flex-1 flex flex-col border border-slate-800 rounded-xl overflow-hidden bg-slate-950">
                    <div className="bg-slate-950 px-4 py-2 border-b border-slate-800 flex justify-between items-center">
                      <span className="text-[10px] font-bold uppercase text-slate-400">{currentQ.language} Editor</span>
                      <span className="text-[9px] text-slate-500">Autosaved continuously</span>
                    </div>
                  ...
                  <div className="shrink-0 flex flex-col gap-2">
                    <button
                      ...
                      className="self-start rounded-lg bg-emerald-600 hover:bg-emerald-500 px-5 py-2 text-xs font-bold text-white transition-all disabled:opacity-50 flex items-center gap-2 cursor-pointer"
                    >
                      {isRunningCode ? (
                        <>
                    ...
                    {runOutput && (
                      <div className="rounded-xl border border-slate-800 bg-black p-4 text-left">
                        <div className="text-[9px] font-bold text-slate-500 uppercase tracking-wider mb-2">Console Output</div>
                        <pre className="text-xs text-slate-300 font-mono whitespace-pre-wrap">{runOutput}</pre>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="border-t border-slate-800 bg-slate-950 p-4 shrink-0 flex items-center justify-between">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-900 px-5 py-2 text-xs font-bold text-slate-350 hover:text-white disabled:opacity-30 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-slate-850 px-5 py-2 text-xs font-bold text-white hover:bg-slate-800 disabled:opacity-30 transition-all cursor-pointer"
            >
      ...
      {showAutoSubmitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 text-center">
          <div className="w-full max-w-sm rounded-2xl border border-rose-900 bg-slate-950 p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-rose-955/50 text-rose-500 mx-auto mb-6">
              <AlertTriangle className="h-6 w-6" />
            </div>
            <h2 className="text-lg font-bold text-white mb-2">Exam Auto-Submitted</h2>
            ...
            <button
              ...
              className="w-full rounded-xl bg-rose-600 hover:bg-rose-500 py-3 text-xs font-bold text-white shadow-lg transition-all cursor-pointer"
            >
              Continue
            </button>
          </div>
      ...
      {showWarningModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 text-center">
          <div className="w-full max-w-sm rounded-2xl border border-rose-900 bg-slate-950 p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <h2 className="text-lg font-bold text-white mb-2">Proctoring Warning</h2>
